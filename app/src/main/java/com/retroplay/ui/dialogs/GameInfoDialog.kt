...
fun GameInfoDialog(
    gameInfo: GameInfo?,
    gameCRC: String?,
    configId: String? = null, // Config ID used for config file name (may differ from CRC for PSX)
    console: String?,
    cheatFile: File?,
    onDismiss: () -> Unit,
    ...
    Dialog(onDismissRequest = onDismiss) {
        Surface(
            ...
            Column(
                ...
                // Per-Game Config button (if game info is available)
                // Use gameName for config (like saves system)
                if (gameInfo != null && console != null && onOpenPerGameConfig != null) {
                    val hasOverride = com.retroplay.config.RetroPlayConfigManager.hasGameConfig(console, gameInfo.name)
                    
                    OutlinedButton(
                        onClick = { 
                            onOpenPerGameConfig(gameInfo.name, gameInfo.name)  // Pass gameName twice (for compatibility)
                        },
...
private fun GameInfoContent(
    ...
    Column {
        ...
        // Config ID (for config file name) - Show if different from CRC
        if (configId != null && configId != gameCRC) {
            InfoRow(
                icon = "⚙️",
                label = "Config ID",
                value = "$configId.cfg",
            ...
            Text(
                text = "Config file: /RetroPlay-Data/config/games/$configId.cfg",
                fontSize = 11.sp,
            ...
            // Same as CRC, but show for clarity
            InfoRow(
                icon = "⚙️",
                label = "Config File",
                value = "$configId.cfg",
        ...
        // Use gameName for config (like saves system)
        val gameNameForConfig = gameInfo.name
        val appliedConfig = RetroPlayConfigManager.getEffectiveConfig(gameInfo.console, gameNameForConfig)
        val hasPerGameOverride = RetroPlayConfigManager.hasGameConfig(gameInfo.console, gameNameForConfig)
...
private fun SmartConfigSection(
    ...
    Column {
        ...
        if (hasPerGameOverride && effectiveConfigId != null) {
            Spacer(modifier = Modifier.height(6.dp))
            Text(
                text = "Per-game override active for config: $effectiveConfigId.cfg",
                fontSize = 12.sp,
