...
(function() {
    ...
    if (!enableThreads) {
        ...
        console.warn('[PSP] Pour activer threads, accédez via l\'app ou http://localhost:7777');
    }
    ...
    if (gameUrl) {
        ...
        const pspControlUrl = useDpad 
            ? `http://${serverIP}:7777/gamelibrary/pspcontroldpad.js`  // D-Pad only
            : `http://${serverIP}:7777/gamelibrary/pspcontrol.js`;     // Analog sticks
    ...
    fetch(`http://${serverIP}:7777/gamedata/psp/gamelist.json`)
        ...
        .then(data => {
            ...
            window.EJS_gameUrl = `http://${serverIP}:7777/gamedata/psp/${encodedGameFile}`;
