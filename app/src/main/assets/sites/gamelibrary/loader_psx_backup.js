...
(function() {
    ...
    if (gameUrl) {
        ...
        // Choose which PSX control script to load (both served locally now)
        const psxControlUrl = useDpad 
            ? `http://${serverIP}:7777/gamelibrary/psxcontroldpad.js`  // D-Pad only
            : `http://${serverIP}:7777/gamelibrary/psxcontrol.js`;     // Analog sticks (local copy)
    ...
    fetch(`http://${serverIP}:7777/gamedata/psx/gamelist.json`)
        .then(response => {
        ...
        .then(data => {
            ...
            window.EJS_gameUrl = `http://${serverIP}:7777/gamedata/psx/${gameFile}`;
            
            console.log('[PSX] Game URL:', window.EJS_gameUrl);

            // Choose which PSX control script to load
            const psxControlUrl = useDpad 
                ? `http://${serverIP}:7777/gamelibrary/psxcontroldpad.js`  // D-Pad only
                : 'https://www.lemon-web.net/includes/psxcontrol.js';      // Analog sticks
