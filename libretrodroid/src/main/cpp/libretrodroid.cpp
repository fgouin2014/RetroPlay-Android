...
void LibretroDroid::onMouseButton(int port, int button, int pressed) {
    LOGD("Received mouse button event: port=%d button=%d pressed=%d", port, button, pressed);
    if (input) {
        input->onMouseButton(port, button, pressed);
    }
}
