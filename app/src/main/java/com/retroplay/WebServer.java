...
public class WebServer {
        ...
        try {
            ...
            if (filePath.endsWith("/gamelist.json")) {
                ...
                if (!file.exists()) {
                    ...
                    String consoleDirPath = "/storage/emulated/0/GameLibrary-Data/" + consoleName;
     ...
     * Utilise GamelistManager amélioré avec métadonnées complètes (CRC32, taille, etc.)
     */
    private void serveAutoGeneratedGamelist(OutputStream outputStream, String consoleName, String method) throws IOException {
        try {
            ...
            Log.d(TAG, "Directory exists: " + consoleDir.exists() + ", isDirectory: " + (consoleDir.exists() ? consoleDir.isDirectory() : "N/A"));
            
            if (!consoleDir.exists() || !consoleDir.isDirectory()) {
                // Si le répertoire n'existe pas avec ce nom, essayer de trouver un alias
                // Ex: "genesis" n'existe pas mais "megadrive" existe
                String normalizedId = ConsoleNameMapper.normalizeToCanonical(consoleName);
                if (!normalizedId.equals(consoleName)) {
                    ...
                    if (gamelibraryDir.exists() && gamelibraryDir.isDirectory()) {
                        java.io.File[] dirs = gamelibraryDir.listFiles(java.io.File::isDirectory);
                        if (dirs != null) {
                            for (java.io.File dir : dirs) {
                                String dirName = dir.getName();
                                String dirNormalized = ConsoleNameMapper.normalizeToCanonical(dirName);
                                if (dirNormalized.equals(normalizedId)) {
                                    consoleDir = dir;
                                    consoleName = dirName; // Utiliser le nom réel du répertoire
                                    Log.i(TAG, "Found console directory by alias: " + dirName + " (for " + consoleName + " -> " + normalizedId + ")");
                                    break;
                                }
                            }
                        }
            ...
            // Passer le nom réel du répertoire pour le scan, mais l'ID canonique pour la config
            ...
            try {
                ...
                Log.i(TAG, "Served improved auto-generated gamelist.json for " + consoleName + " with " + gamesArray.length() + " games (with CRC32, size, metadata)");
            } catch (Exception e) {
    ...
    private void serveConsolesAPI(OutputStream outputStream, String method) throws IOException {
        try {
            ...
            if (gamelibraryDir.exists() && gamelibraryDir.isDirectory()) {
                ...
                if (directories != null) {
                    for (java.io.File dir : directories) {
                        ...
                        // Normaliser le nom du répertoire avec ConsoleNameMapper pour gérer les noms alternatifs
                        ...
                        if (hasGamelist || hasRoms) {
                            // Utiliser le nom normalisé pour la détection, mais garder le nom original du répertoire
                            ...
                            if (consoleInfo != null) {
                                ...
                                Log.d(TAG, "Console detected: " + dirName + " (normalized: " + normalizedDirName + ") - " + 
            ...
            // "Broken pipe" est normal quand le client ferme la connexion avant la fin de l'envoi
            if (e.getMessage() != null && e.getMessage().contains("Broken pipe")) {
    ...
    private org.json.JSONObject detectConsoleConfig(java.io.File consoleDir, String dirName) {
        try {
            ...
            if (configFile.exists()) {
                try {
                    ...
                    consoleInfo.put("fullName", userConfig.optString("fullName", 
                        ...
                        ConsoleNameMapper.getFullName(canonicalId) : canonicalId.toUpperCase()));
            ...
            if (autoDetectedConfig != null) {
                ...
                Log.d(TAG, "Auto-detected config from cores.json for: " + dirName + " (canonical: " + canonicalId + ")");
    ...
    private String getConsoleFullName(String consoleId) {
        ...
        switch (consoleId.toLowerCase()) {
            ...
            case "nes":
            case "famicom":
            case "fc": return "Nintendo Entertainment System";
            case "snes":
            case "sfc":
            case "superfamicom": return "Super Nintendo Entertainment System";
            ...
            case "megadrive":
            case "md":
            case "mega drive": return "Sega Genesis / Mega Drive";
            case "mastersystem":
    ...
    private String getDefaultCore(String consoleId) {
        ...
        switch (canonicalId) {
            ...
            case "32x": return "picodrive";
            case "segacd": return "genesis_plus_gx";
     ...
     * @return JSONObject avec la config auto-détectée, ou null si aucun core compatible
     */
    private org.json.JSONObject autoDetectFromCoresJson(String consoleDirName) {
        try {
            ...
            Log.i(TAG, "Auto-detected core for " + consoleDirName + ": " + defaultCoreName + " (" + defaultCoreId + ")");
            return consoleInfo;
    ...
    private java.util.List<String> getCorePatternsForConsole(String consoleKey) {
        ...
        switch (canonicalId) {
            ...
            case "cps2":
                ...
                break;
            default:
                // Pour les consoles inconnues, essayer de chercher le nom de la console dans le nom du core
