// PSP Controller with D-Pad (NO ANALOG STICKS) - Based on PSPcontrol.js
function ensurePSPFonts() {
  ...
  add('PSP-preconnect-apis',    { rel: 'preconnect', href: 'https://fonts.googleapis.com' });
  add('PSP-preconnect-gstatic', { rel: 'preconnect', href: 'https://fonts.gstatic.com', crossOrigin: 'anonymous' });

  const sheet = add('PSP-fonts', {
    rel: 'stylesheet',
...
window.EJS_VirtualGamepadSettings = [
  // Shoulders (TOP)
  { type:"button", text:"L1", id:"PSP-l1", location:"top",   left:12,   top:-40, fontSize:16, bold:true, block:true, input_value:10 },
  { type:"button", text:"L2", id:"PSP-l2", location:"top",   left:100,  top:-40, fontSize:16, bold:true, block:true, input_value:12 },
  { type:"button", text:"R1", id:"PSP-r1", location:"top",   right:100, top:-40, fontSize:16, bold:true, block:true, input_value:11 },
  { type:"button", text:"R2", id:"PSP-r2", location:"top",   right:12,  top:-40, fontSize:16, bold:true, block:true, input_value:13 },

  // D-PAD instead of zone (digital directional pad)
  { type:"dpad", location:"left", left:"52%", top:"42%", joystickInput:false, inputValues:[4,5,6,7] },

  // Face buttons
  { type:"button", text:"△", id:"PSP-tri",   location:"right", left: 84, top: 34,  fontSize:28, bold:true, input_value:9 },
  { type:"button", text:"○", id:"PSP-cir",   location:"right", left:132, top: 88, fontSize:28, bold:true, input_value:8 },
  { type:"button", text:"✕", id:"PSP-cross", location:"right", left: 84, top: 122, fontSize:28, bold:true, input_value:0 },
  { type:"button", text:"□", id:"PSP-sq",    location:"right", left: 36, top: 88, fontSize:28, bold:true, input_value:1 },

  // Start / Select (CENTER)
  { type:"button", text:"SELECT", id:"PSP-sel",   location:"center", left: -36,  bottom: 2, fontSize:12, bold:true, block:true, input_value:2 },
  { type:"button", text:"START",  id:"PSP-start", location:"center", left: 64, bottom: 2, fontSize:12, bold:true, block:true, input_value:3 }
];
...
(function styleAndPlaceVPad(){
  ...
  function apply(root){
    $all().forEach(node => {
      ...
      if (!label || node.__PSPStyled) return;
  ...
  function addPressEffects(){
    ...
    faces.forEach(sym => {
      ...
      if (!box || box.__PSPPressBound) return;
  ...
  ensurePSPFonts().finally(lockVpadFont);
