// MAME Arcade Loader Configuration - OPTIMIZED
(function() {
    const serverIP = window.location.hostname;
    const urlParams = new URLSearchParams(window.location.search);
    const gameUrl = urlParams.get('game');
    const gameSlug = urlParams.get('slug');
    const coreParam = urlParams.get('core'); // Support du paramètre URL

    window.EJS_player = "#game";
    
    // Utiliser le core spécifié ou mame2010 par défaut (bonne compatibilité)
    window.EJS_core = coreParam || "mame2010";
    console.log('Using core:', window.EJS_core, coreParam ? '(from URL)' : '(default)');
    ...
    fetch(`http://${serverIP}:8888/gamedata/mame/gamelist.json`)
        ...
        .then(data => {
            ...
            window.EJS_gameUrl = `http://${serverIP}:8888/gamedata/mame/${gameFile}`;
            const loaderScript = document.createElement('script');
            loaderScript.src = `http://${serverIP}:8888/gamedata/data/loader.js`;
            document.body.appendChild(loaderScript);
        })
