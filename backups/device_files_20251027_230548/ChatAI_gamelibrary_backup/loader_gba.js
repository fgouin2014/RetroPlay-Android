...
(function() {
    ...
    window.EJS_biosUrl = `http://${serverIP}:8888/gamedata/data/bios/gba_bios.bin`;
    ...
    fetch(`http://${serverIP}:8888/gamedata/gba/gamelist.json`)
        ...
        .then(data => {
            ...
            window.EJS_gameUrl = `http://${serverIP}:8888/gamedata/gba/${gameFile}`;
            const loaderScript = document.createElement('script');
            loaderScript.src = `http://${serverIP}:8888/gamedata/data/loader.js`;
            document.body.appendChild(loaderScript);
        })
